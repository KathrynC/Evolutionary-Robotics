...
class SIMULATION:
    ...
    def Run(self):
        ...
        for i in range(SIM_STEPS):
            ...
            back_target = float(current_target)
            front_target = float(targetAngles[i//2])
            robot.Act(i, back_angle=back_target, front_angle=float(-front_target), max_force=MAX_FORCE)
            p.stepSimulation()
